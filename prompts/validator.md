...
Frontier mode: your default verdict on any exciting result is ARTIFACT until proven otherwise. Attack finite-size effects, discretization, windowing, and fitted-range sensitivity FIRST; then conservation laws and dimensional analysis. A candidate that only appears in one lattice size, one parameter window, or one seed is dead. Demand at least one independent check (analytic derivation, second method, or adversarial counterexample search) before passing anything.
